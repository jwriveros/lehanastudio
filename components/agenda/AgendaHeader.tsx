      ...
      <header className="relative z-50 flex flex-col gap-3 border-b border-zinc-200/80 bg-white p-3 sm:p-4 dark:border-zinc-800 dark:bg-zinc-900/90 backdrop-blur-md font-sans text-zinc-900 dark:text-zinc-100">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* 1. Botón compacto '+' (Con el degradado Rose exclusivo de Lehana Studio) */}
            <button
              type="button"
              onClick={() => setIsCreateOpen(true)}
              className="flex h-9 w-9 items-center justify-center rounded-2xl bg-gradient-to-r from-rose-500 to-pink-500 text-white shadow-md shadow-rose-500/20 transition-all hover:from-rose-600 hover:to-pink-600 active:scale-95 cursor-pointer flex-shrink-0"
              title="Crear Reserva"
            ...
            <button
              type="button"
              onClick={onToday}
              className="flex h-9 items-center gap-1.5 rounded-2xl border border-zinc-200 bg-zinc-50 px-3.5 text-xs font-bold text-zinc-700 transition-all hover:bg-zinc-100 active:scale-95 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200 dark:hover:bg-zinc-800 cursor-pointer flex-shrink-0"
            >
            ...
            <div className="flex h-9 items-center rounded-2xl bg-zinc-100/80 dark:bg-zinc-950 border border-zinc-200/60 dark:border-zinc-800/80 p-0.5 flex-shrink-0">
              <button
                type="button"
                onClick={onPrev}
                className="rounded-xl p-1.5 text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-100 hover:bg-white dark:hover:bg-zinc-800 transition-all cursor-pointer"
                aria-label="Anterior"
              >
                <ChevronLeft size={15} />
              </button>
              ...
              <div className="relative">
                <button
                  ...
                  className="px-3 py-1 text-xs sm:text-xs font-black uppercase tracking-wider text-zinc-800 dark:text-zinc-100 hover:text-rose-500 transition-colors cursor-pointer"
                >
                  {label}
                </button>
                ...
                {showPicker && (
                  <>
                    ...
                    />
                    <div className="fixed top-28 left-1/2 -translate-x-1/2 sm:absolute sm:top-full sm:mt-2 sm:left-auto sm:right-0 sm:translate-x-0 z-[200] w-[285px] max-w-[calc(100vw-2rem)] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-2xl rounded-3xl p-3.5 animate-in fade-in zoom-in-95 duration-150 text-zinc-900 dark:text-zinc-100">
                      <DayPicker
                        mode="single"
                        ...
                        classNames={{
                          ...
                          caption_label: "text-xs font-extrabold uppercase tracking-wider text-zinc-900 dark:text-zinc-100",
                          nav: "flex items-center gap-1",
                          button_previous: "p-1 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-900 dark:hover:text-white cursor-pointer",
                          button_next: "p-1 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-900 dark:hover:text-white cursor-pointer",
                          month_grid: "w-full border-collapse space-y-1",
                          weekdays: "flex justify-between border-b border-zinc-100 dark:border-zinc-800 pb-1 mb-1",
                          ...
                          day: "h-8 w-8 text-xs font-bold rounded-xl transition-all hover:bg-rose-50 dark:hover:bg-rose-950/30 flex items-center justify-center text-center cursor-pointer",
                          selected: "bg-rose-500 text-white hover:bg-rose-600 shadow-md shadow-rose-500/20 font-black",
                          today: "text-rose-500 font-black ring-1 ring-rose-500/40 rounded-xl",
                        }}
                        components={{
                          Chevron: (props) => {
            ...
            <div className="flex h-9 overflow-hidden rounded-2xl border border-zinc-200/80 bg-zinc-100/80 p-0.5 text-xs dark:border-zinc-800 dark:bg-zinc-950 flex-shrink-0">
              {(["day", "week", "month"] as const).map((v) => (
                <button
                  ...
                  key={v}
                  onClick={() => setView(v)}
                  className={`px-3 py-1 transition-all font-extrabold uppercase rounded-xl text-[10px] tracking-wider cursor-pointer ${
                    view === v
                      ? "bg-white dark:bg-zinc-800 text-rose-500 dark:text-rose-400 shadow-xs"
          ...
          {/* FILTROS GLOBALES (Sedes, Estados, Especialistas) */}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-zinc-100 dark:border-zinc-800/80">
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-2xl text-[10px] font-extrabold uppercase tracking-wider transition-all cursor-pointer ${
                  weekBlock === "block1"
                    ? "bg-rose-500 text-white shadow-md shadow-rose-500/20"
                    : "bg-zinc-100 dark:bg-zinc-950 text-zinc-500 hover:bg-zinc-200 dark:hover:bg-zinc-800"
                }`}
              >
