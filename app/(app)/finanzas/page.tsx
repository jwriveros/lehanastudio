...
export default function FinanzasPage() {
  ...
  useEffect(() => { loadData(); }, [dateRange, refreshKey]);
  ...
  return (
    <main className="p-4 lg:p-6 w-full space-y-6 animate-in fade-in duration-700 text-zinc-900 dark:text-zinc-100">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b dark:border-zinc-800 pb-6">
        <div>
          <h1 className="text-4xl font-black uppercase italic tracking-tighter leading-none">Estado Financiero</h1>
          <div className="flex items-center gap-2 mt-2">
             <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
             <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Balance de Lehana Studio</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="mt-1"><DailyPaymentsReport /></div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2.5rem] border dark:border-zinc-800 shadow-sm relative overflow-hidden group">
          ...
          <div className="mt-4"><span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Ingresos Periodo</span><p className="text-3xl font-black italic mt-1">${totalVentas.toLocaleString()}</p></div>
        </div>
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2.5rem] border dark:border-zinc-800 shadow-sm relative overflow-hidden group">
          ...
          <div className="mt-4"><span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Gastos Periodo</span><p className="text-3xl font-black italic mt-1">${totalGastosPeriodo.toLocaleString()}</p></div>
        </div>
        <div className="bg-zinc-900 dark:bg-white p-6 rounded-[2.5rem] shadow-2xl text-white dark:text-zinc-900">
          ...
          <div className="mt-4"><span className="text-[10px] font-black opacity-60 uppercase tracking-widest">Utilidad Real</span><p className="text-3xl font-black italic mt-1">${(totalVentas - totalGastosPeriodo).toLocaleString()}</p></div>
        </div>
      </section>
      ...
      <section className="bg-white dark:bg-zinc-900 p-8 rounded-[3rem] border dark:border-zinc-800 shadow-sm">
        <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-sm font-black uppercase tracking-[0.2em] italic text-zinc-900 dark:text-zinc-100">Rendimiento Histórico (5 Meses)</h2>
        </div>
      ...
      {/* VENTANA MODAL FLOTANTE DE REGISTRO */}
      {isAdding && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-zinc-900 w-full max-w-lg rounded-[2.5rem] shadow-2xl border dark:border-zinc-800 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-8 py-6 border-b dark:border-zinc-800 flex justify-between items-center bg-zinc-50 dark:bg-zinc-800/50">
              <h2 className="text-sm font-black uppercase tracking-widest italic text-indigo-600">Nuevo Registro de Gasto</h2>
              <button onClick={() => setIsAdding(false)} className="p-2 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-full transition-colors text-zinc-500"><X size={20}/></button>
            </div>
            
            <form onSubmit={handleSubmit} className="p-8 space-y-4">
              <div className="space-y-4">
                <input type="text" placeholder="Concepto (Ej: Arriendo)" value={form.concepto} onChange={e => setForm({...form, concepto: e.target.value})} className="w-full bg-zinc-50 dark:bg-zinc-800 p-4 rounded-2xl font-bold ring-1 ring-zinc-200 dark:ring-zinc-700 outline-none focus:ring-2 focus:ring-indigo-500 text-zinc-900 dark:text-zinc-100" />
                ...
                <div className="relative text-zinc-900 dark:text-zinc-100">
                  ...
                  <input type="number" placeholder="Valor" value={form.valor} onChange={e => setForm({...form, valor: e.target.value})} className="w-full bg-zinc-50 dark:bg-zinc-800 p-4 pl-10 rounded-2xl font-bold outline-none ring-1 ring-zinc-200 dark:ring-zinc-700 focus:ring-2 focus:ring-indigo-500" />
                </div>
                ...
                <div className="flex items-center gap-3 p-4 bg-zinc-50 dark:bg-zinc-800 rounded-2xl border dark:border-zinc-700">
                  ...
                  <span className="text-[11px] font-black uppercase text-zinc-500">¿Automatizar mensualmente?</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input type="date" value={form.fecha} onChange={e => setForm({...form, fecha: e.target.value})} className="bg-zinc-50 dark:bg-zinc-800 p-4 rounded-2xl text-[10px] font-bold outline-none text-zinc-900 dark:text-zinc-100" />
                  <select value={form.especialista} onChange={e => setForm({...form, especialista: e.target.value})} className="bg-zinc-50 dark:bg-zinc-800 p-4 rounded-2xl text-[10px] font-bold outline-none text-zinc-900 dark:text-zinc-100">
                    <option value="">General</option>
                    {specialists.map(s => <option key={s.name} value={s.name}>{s.name}</option>)}
                  </select>
                  <select value={form.metodo_pago} onChange={e => setForm({...form, metodo_pago: e.target.value})} className="col-span-2 bg-zinc-50 dark:bg-zinc-800 p-4 rounded-2xl text-[10px] font-bold outline-none text-zinc-900 dark:text-zinc-100">
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-6 border dark:border-zinc-800 shadow-sm space-y-6">
        <div className="flex justify-between items-center px-2">
          ...
          <button onClick={() => setShowFixedManager(!showFixedManager)} className="text-[10px] font-black uppercase text-indigo-600 underline decoration-2 underline-offset-4 decoration-indigo-500/30">
        ...
        <div className="max-h-[500px] overflow-y-auto pr-2 space-y-3 custom-scrollbar text-zinc-900 dark:text-zinc-100">
          {showFixedManager ? (
             recurringRules.map(rule => (
              <div key={rule.id} className="bg-amber-50/50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 p-5 rounded-3xl flex justify-between items-center animate-in fade-in">
                <div><span className="text-sm font-black block uppercase">{rule.concept || rule.concepto}</span><span className="text-[9px] font-bold text-amber-600 uppercase">Día {rule.dia_pago} del mes</span></div>
            ...
            expenses.length === 0 ? (
              <p className="text-center py-20 text-zinc-400 text-[10px] font-bold italic uppercase font-black">Sin movimientos registrados</p>
            ) : (
              expenses.map((exp) => (
